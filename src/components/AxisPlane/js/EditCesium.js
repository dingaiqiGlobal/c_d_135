...
class EditCesium {
  ...
  createMoveAxis() {
    ...
    let positions = [
      center,
      new Cesium.Cartesian3(
        (this.xAxisLength ?? lineLength) + center.x,
        center.y,
        center.z
      ),
    ];
    ...
    positions = [
      ...
      new Cesium.Cartesian3(
        center.x,
        -(this.yAxisLength ?? lineLength) + center.y,
        center.z
      ),
    ];
    this.yAxis = plc.add({
    ...
    positions = [
      ...
      new Cesium.Cartesian3(
        center.x,
        center.y,
        (this.zAxisLength ?? lineLength) + center.z
      ),
    ];
    this.zAxis = plc.add({
      positions: positions,
  ...
  transform(startPosition, endPosition) {
    ...
    if (this.activePrimitive === this.originPoint) {
      ...
      const s =
        -_offset.sxyz /
          Cesium.Cartesian3.distance(...this.scaleAxis.positions) +
        1;
  ...
  computeOffset(helper, startPosition, endPosition, offset) {
    ...
    for (const axis of activeAxis) {
      ...
      const pixelList = cartList.map((_) =>
        LonLat.toPixel(_, helper.viewer.scene)
      );
